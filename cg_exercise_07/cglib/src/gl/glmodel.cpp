	...
	: flags(_flags)
{
	...
	for(unsigned i = 0; i < obj_file.getModelCount(); i++) {
		...
		for(const auto &surf: modelSurfaces) {
			...
			// get texture
			const auto iter = surf->material->additionalInfo.find("map_Kd");

			if (iter != surf->material->additionalInfo.end())
			{
				surfaceInfo.gl_texBuffer = textures.getGLTexture(iter->second);
			}
			else {
				surfaceInfo.gl_texBuffer = textures.getGLTexture("assets/empty.png");
			}
...
draw()
{
	...
	{
		glBindTexture(GL_TEXTURE_2D, faceData.gl_texBuffer);
