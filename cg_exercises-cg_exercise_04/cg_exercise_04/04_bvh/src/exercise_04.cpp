...
//	Image::create_gaussian_kernel_2d(1, kernel_size, kernel);
	int kernelCenter = kernel_size / 2;
...
{/*{{{*/
	...
	if (node.num_triangles > BVH::MAX_TRIANGLES_IN_LEAF) {
		...
		build_bvh(node.left, first_triangle_idx, newnumtriangles, depth + 1);
		build_bvh(node.right, first_triangle_idx + newnumtriangles, num_triangles - newnumtriangles, depth + 1);
	}
}/*}}}*/
	...
	std::complex<float>	* reconstruction)
{
	...
	for (int k = 0; k < M ; k++) {
		for (int l = 0; l < N; l++) {
			...
			for (int m = 0; m < M; m++ ) {
				for (int n = 0; n < N; n++) {	
					std::complex<float> fourierkoeffi = *spectrum;
					spectrum ++;
