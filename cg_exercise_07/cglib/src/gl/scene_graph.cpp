...
MAYBE_REFERENCE_FUNCTION(buildSphereFlakeSceneGraph)(const std::shared_ptr<GLObjModel>& model, float sphere_model_radius, float size_factor, int number_of_remaining_recursions)
{
	...
	glm::mat4 child_scale_mat = glm::scale(glm::vec3(size_factor));
	glm::mat4 child_translation_mat = glm::translate(glm::vec3(0, child_distance_from_center, 0));
	
	glm::mat4 child_rotation_matrices[5];
	// top
	child_rotation_matrices[0] = glm::mat4();
	// left
	child_rotation_matrices[1] = glm::rotate(float(M_PI) / 2.f, glm::vec3(0, 0, 1));
	// right
	child_rotation_matrices[2] = glm::rotate(float(M_PI) / 2.f, glm::vec3(0, 0, -1));
	// front
	child_rotation_matrices[3] = glm::rotate(float(M_PI) / 2.f, glm::vec3(1, 0, 0));
	// back
	child_rotation_matrices[4] = glm::rotate(float(M_PI) / 2.f, glm::vec3(-1, 0, 0));
...
MAYBE_REFERENCE_FUNCTION(animateSphereFlake)(SceneGraphNode& root, float angle_increment)
{
	...
	auto rotation = glm::rotate(angle_increment, glm::vec3(0, 1, 0));
