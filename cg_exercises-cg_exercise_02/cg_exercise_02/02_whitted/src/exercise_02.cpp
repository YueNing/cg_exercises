    ...
    float* t)                       // output parameter which contains distance to the hit point
{
	...
	float a,b,c,diskri,t1,t2;
	a = glm::dot(ray_direction, ray_direction);
	b = 2*glm::dot(ray_direction, ray_origin - center);
	c = glm::dot(ray_origin - center, ray_origin - center) - radius*radius;
	diskri = b*b - 4*a*c;
	t1 = 0.5*(-b - sqrt(diskri))/a;
	t2 = 0.5*(-b + sqrt(diskri))/a;
		...
		) const
{
	...
	glm::vec3 P = Light::getPower();
	float alpha = SpotLight::falloff;
	float Odotd = glm::dot(omega, SpotLight::direction);
	glm::vec3 E = P * (alpha + 2) * std::max(0.f, powf(Odotd, alpha));
	// return glm::vec3(0.f);
	return E;
	...
	glm::vec3 const& V)			// view vector (already normalized)
{
	...
	for (auto& light_uptr : data.context.scene->lights) 
	{
		...
		glm::vec3 Rl = 2.f*N*(glm::dot(L, N))-L;
		float distance = glm::dot(P-l, P-l);

		float visibility = 1.f;
		if (data.context.params.shadows) {
			// TODO: check if light source is visible
			if(!(visible(data, P, l))){visibility = 0.f;}
		}

		glm::vec3 diffuse(0.f);
		if (data.context.params.diffuse) {
			// TODO: compute diffuse component of phong model
			diffuse = mat.k_d*visibility*light->getEmission(-L)*std::max(0.f, glm::dot(N, L));
		}

		glm::vec3 specular(0.f);
		if (data.context.params.specular) {
			// TODO: compute specular component of phong model
			specular = mat.k_s*visibility*light->getEmission(-L)*std::max(0.f, powf(glm::dot(Rl, V), mat.n));
		}
		...
		contribution += ambient * light->getEmission(-L)/distance + (diffuse + specular)/distance;
	}
	...
	float eta)					// the relative refraction index
{
	if (data.context.params.fresnel) {
		...
		// contribution = F*evaluate_reflection(data, depth, P, N, V);
		// return contribution;
		return F*evaluate_reflection(data, depth, P, N, V) + (1.f-F)*evaluate_transmission(data, depth, P, N, V, eta);
	}
	else {
...
// CG_REVISION 060dc4223863c6e5d0f0f58ce5ec6e133a5737e4
