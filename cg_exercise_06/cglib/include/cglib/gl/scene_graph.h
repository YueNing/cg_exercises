...
{
public:
	glm::mat4 node_to_parent;
	glm::mat4 parent_to_node;
	std::vector<std::shared_ptr<SceneGraphNode>> children;
	std::shared_ptr<GLObjModel> model;

	void collectTransformedModels(std::vector<TransformedModel>& transformed_models, const glm::mat4& parent_to_world = glm::mat4(), const glm::mat4& world_to_parent = glm::mat4()) const;
};
