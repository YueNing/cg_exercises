...
void main (void)
{
	...
	vec3 textureKoordinaten = pos_shadowmap_space.xyz / pos_shadowmap_space.w;
	textureKoordinaten = textureKoordinaten * 0.5 + 0.5;

	// TODO: determine visibility for the fragment
	float visibility = 1.0;
	float closeDepth = texture(shadow_map, textureKoordinaten.xy).r;
	float currentDepth = textureKoordinaten.z;
	visibility = currentDepth - shadow_bias>closeDepth?0.0:1.0;
	vec3 color = vec3(0);
	if (visibility > 0.0) {
