...
int CubeRenderer::
_initialize(const Parameters &parameters)
{
...
_draw()
{
	...
	// glossy
	shader_transform(env_light, glm::translate(glm::vec3(3.0f, 0.0f, 0.0f)) * glm::rotate(float(M_PI), glm::vec3(0.0f, 1.0f, 0.0f)), glm::vec3(0.0f), glm::vec3(0.7f, 0.6f, 0.5f));
	monkey->draw();

	// fresnel
	shader_transform(env_light, glm::translate(glm::vec3(0.0f, 0.0f, 0.0f)) * glm::rotate(float(M_PI), glm::vec3(0.0f, 1.0f, 0.0f)), glm::vec3(1.0f, 0.2f, 0.1f), glm::vec3(0.7f, 0.6f, 0.5f));
	monkey->draw();

	// diffuse
	shader_transform(env_light, glm::translate(glm::vec3(-3.0f, 0.0f, 0.0f)) * glm::rotate(float(M_PI), glm::vec3(0.0f, 1.0f, 0.0f)), glm::vec3(1.0f, 0.2f, 0.1f), glm::vec3(0.0f));
	monkey->draw();
...
_initialize(const Parameters &parameters)
{
	...
	hdrRenderer.initialize(parameters);
...
_draw()
{
	...
	auto VP = projection * camera->get_view_matrix(Camera::Mono);
	auto model_matrix = glm::scale(glm::vec3(0.01f));
